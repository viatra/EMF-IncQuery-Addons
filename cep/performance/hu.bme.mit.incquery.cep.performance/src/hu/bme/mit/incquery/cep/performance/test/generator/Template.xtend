...
class Template {
	...
	def generate(String filename, List<List<String>> perm){
		this.writer = new FileWriter(filename)
		writer.append(head)
		generateComplexEvents(perm)
	...
	def public generateComplexEvents(List<List<String>> perm){
		var i = 1
		
		for(p : perm){
			writer.append(generateComplexEvent(i, generateDefinition(p)))
			i=i+1
		}
	}
	
	def private generateDefinition(List<String> atomicEventList){
		Joiner::on(" -> ").join(atomicEventList)
	}
